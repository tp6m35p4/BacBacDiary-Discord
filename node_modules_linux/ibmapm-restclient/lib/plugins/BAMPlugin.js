...
function sendOne(task) {
    ...
    if (IBAM_options) {
        ...
        if (task.type.indexOf('resources:') === 0) {
            ...
            if (task.payload.entityTypes[0] === 'serviceEndpoints') {
                task.url = wf_url +
...
function parseIngressURL() {
    ...
    if (process.env.IBAM_INGRESS_URL) {
        ...
        if (process.env.ICAM_KEYFILE) {
            if (!process.env.ICAM_KEYFILE.toLowerCase().endsWith('.p12')) {
                process.env.ICAM_KEYFILE = process.env.ICAM_KEYFILE + '.p12';
            }
...
function pollMountSecret() {
    ...
    let ingressUrl = globalEnv.IBM_APM_SERVER_INGRESS_URL;
    let tenantId = globalEnv.APM_TENANT_ID;
    let agentmgmtUrl = globalEnv.AGENTMGMT_SERVICE_BASE_PATH;
    let metricUrl = globalEnv.METRICS_SERVICE_BASE_PATH;
    let resMgmtUrl = globalEnv.RESOURCEMGMT_SERVICE_BASE_PATH;
    logger.debug('Poll the server configure from mount folder:', ingressUrl,
        tenantId, agentmgmtUrl, metricUrl, resMgmtUrl);
    if (agentmgmtUrl) {
    ...
    if (ingressUrl) {
        ...
        if (pluginConfig.globalEnv.ICAM_KEYFILE_PSWD) {
            IBAM_options.addtionalOptions.passphrase = pluginConfig.globalEnv.ICAM_KEYFILE_PSWD;
        }
...
function sendMetadata() {
    ...
    try {
        ...
        let mdinst = require('../../etc/ResourceTypes/serviceInstance_1.2_ResourceType.json');
